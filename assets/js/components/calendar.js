...
var JSCalendar = function () {
    function JSCalendar(elem, options) {
        ...
        this.elem = elem;
        this.id = elem.id;
        this.options = Object.assign(JSCalendar.defaultOptions, options || {
            defaultOptions: true
        });
    ...
    _createClass(JSCalendar, [{
        ...
        value: function moveCell(ev) {
            ...
            if (!ev) {
                return this.fire("cellDidNotMove", {
                    event: ev,
                    reason: new Error("moveCell was fired without an event")
                });
            }
            ...
            if (this.state.view == "day") {
                ...
                if (newgap != ev.gapcell) {
                    ...
                    ev.dayElem.style.top = newtop + "px";
                    this.fire('cellDidNotMove', {
                        event: ev,
                        reason: new Error("Distance between click and drop not big enough")
                    });
                }
            } else if (newPos && newPos != oldPos) {
                this.fire('cellWillMove', {
                    event: ev,
                    oldPosition: oldPos,
                    newPosition: newPos
                });
                ...
                if (_index != -1) {
                    ...
                    this.fire('cellDidNotMove', {
                        event: ev,
                        reason: new Error("Could not find cell in matrix")
                    });
                }
            } else {
                this.fire('cellDidNotMove', {
                    event: ev,
                    reason: new Error("Position did not change")
                });
            }
        }
    }, {
